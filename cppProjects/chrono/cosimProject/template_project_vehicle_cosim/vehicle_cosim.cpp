...
class MyTerrain : public ChVehicleCosimTerrainNode {
  public:
    ...
    virtual void UpdateRigidProxy(unsigned int i, BodyState& spindle_state) override;

    // Collect cumulative contact force and torque on the wheel proxy body for the i-th tire.
    virtual void GetForceRigidProxy(unsigned int i, TerrainForce& wheel_contact) override;
...
void MyTerrain::UpdateRigidProxy(unsigned int i, BodyState& spindle_state) {
...
void MyTerrain::GetForceRigidProxy(unsigned int i, TerrainForce& wheel_contact) {
